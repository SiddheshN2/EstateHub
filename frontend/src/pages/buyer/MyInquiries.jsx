...
import { myInquiriesStyles as s } from "../../style/seller/myInqiriesStyle";
...
const MyInquiries = () => {
  ...
  const handleRespond = async (id, status) => {
    try {
      await axios.patch(
        `${API_URL}/api/inquiry/${id}/respond`,
  ...
  const handleStartChat = async (inq) => {
    ...
    const res = await axios.post(
      `${API_URL}/api/chat/start`,
      {
        ...
        buyerId: inq.buyer?._id,   // seller must send buyerId
      },
      { headers: { Authorization: `Bearer ${token}` } }
    );
            ...
            {inquiries.map((inq) => (
              <div key={inq._id} className={s.inquiryCard}>
                    ...
                    <div className={s.meta}>
                      ...
                      {!isSeller && (
                        <div className={s.flexItemsCenterGap2}>
                          ...
                          {inq.status === "accepted" &&
                            "Seller accepted inquiry"}
                          {inq.status === "rejected" && "Inquiry rejected"}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className={s.actions}>
                  ...
                  {isSeller && inq.status === "pending" && (
                    <>
                      <button
                        onClick={() => handleRespond(inq._id, "accepted")}
                      ...
                      <button
                        onClick={() => handleRespond(inq._id, "rejected")}
                        className={s.btnOutline}
                      >
                        Reject
                      </button>
